...
% Version to locate 1 event at a time in 3d using travel-time tables
clear
...
load(name_mapping,'CC1_ind','max_samlag','pair1','x_no','y_no','z_no')
...
Fs = 100;                           % Sampling frequency for re-sampling
...
% Cleaning
clearvars -except CC1_ind max_samlag pair1 x_no y_no z_no grid0 ...
...
Loct = [str2double(tim_st(7:10)) str2double(tim_st(4:5)) str2double(tim_st(1:2)) ...
    ...
    lat lon maxp(3) max(prod_prob(:)) num_env 0 0 max(uncs(1:2)) uncs(3)];

% ---------------------------------------------------------------------
% Plotting
% ---------------------------------------------------------------------
%plot_env(env_trans,tlag1,pair2,id_sel)
plot_bkproj_prob_3d( rang, prod_prob, sta, files(filenum).name, 0)
...
str = sprintf('%i %i %i %i %i %2.3f %2.6f %2.6f %2.3f %2.2f %i %i %i %2.3f %2.3f\n',Loct');
disp(str)
