...
if calc == 1
    ...
    tlag1 = -lag:lag;
    [CC1_ind,alpha] = mapping_var_3d(x_no,y_no,z_no,pair1,Npair1,rang(1,1):rang(1,3):rang(1,2),...
        rang(2,1):rang(2,3):rang(2,2),rang(3,1):rang(3,3):rang(3,2),sta,vel,Fs,tlag1);

    % Save the mapping operator for future uses if needed
    save(name_mapping,'x_no','y_no','z_no','CC1_ind','alpha',...
...
[env_trans,tlag1,sam_a,~] = makesyn_3d(sta,vel,lag,Fs,x_a,y_a,z_a,func,sig,unc,noise_std,bins,trans_func_all);
