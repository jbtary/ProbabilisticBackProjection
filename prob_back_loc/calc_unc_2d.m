...
%   rang: grid variable with (x0 xmax dx, y0 ymax dy)
% 
% Output:
%   uncs: uncertainties in x and y (same units as rang)
%   uncs_coord: coordinates of the uncertainty zone (same units as rang)

function [uncs,uncs_coord] = calc_unc_2d(prod_prob,fact,rang)
...
pdf_fin = pdf_fin(:);
pdf_fin(inds(1:iv)) = 1;
pdf_fin = reshape(pdf_fin,[size(prod_prob,1) size(prod_prob,2)]);

[iy,ix] = ind2sub(size(pdf_fin),find(pdf_fin > 0));
uncs = [(max(ix)-min(ix))*rang(1,3) (max(iy)-min(iy))*rang(2,3)];
uncs_coord = [(min(ix)-1)*rang(1,3) (max(ix)-1)*rang(1,3);...
    (min(iy)-1)*rang(2,3) (max(iy)-1)*rang(2,3)];
