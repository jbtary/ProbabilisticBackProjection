% Make synthetic data for tremor location, for a 3D velocity model and
% corresponding travel-time tables
% 
% Needs: cons_pair1.m, read_tt.m
% Input:
...
%   wave: wave type to load the travel-time tables
%   rang: grid characteristics (min, max, resolution for 3 rows
%   corresponding to x, y, z) (same unit as in sta)
%   lag: max lag to consider in samples (must be larger than max lag between stations)
%   Fs: sampling frequency in Hz
%   x_a,y_a,z_a: position of the fake source (same unit as station
%   coordinates, and need to be on a grid node)
...
function [env_trans,tlag1,sam_a,alpha] = makesyn_tt(sta,sta_name,path1,wave,...
    rang,lag,Fs,x_a,y_a,z_a,func,sig,unc,nstd,bin_val,trans_func_all)
...
% Grid axes
xaxis = rang(1,1):rang(1,3):rang(1,2);
yaxis = rang(2,1):rang(2,3):rang(2,2);
zaxis = rang(3,1):rang(3,3):rang(3,2);

% Seems necessary even though it should not...
xaxis = round(xaxis,1); yaxis = round(yaxis,1); zaxis = round(zaxis,1);
...
for ii = 1:size(pair1,1)
    ...
    % Loading travel-time tables (need to have same grid characteristics as
    % in rang variable)
    filename1 = dir([path1 '*.' wave '.' sta_name{pair1(ii,1),1} '.time.hdr']);
    filename1 = filename1.name(1:end-4);
    filename2 = dir([path1 '*.' wave '.' sta_name{pair1(ii,2),1} '.time.hdr']);
    filename2 = filename2.name(1:end-4);
    [~,ttsta1,~,~] = read_tt(path1,filename1);
    [~,ttsta2,~,~] = read_tt(path1,filename2);

    % Fake xcorr lag data: 1st step
    diff_tt = ttsta1(xaxis == x_a,yaxis == y_a,zaxis == z_a) - ...
        ttsta2(xaxis == x_a,yaxis == y_a,zaxis == z_a); % Time difference at source location
    sam_a(ii) = round(diff_tt*Fs); % Corresponding sample timing
    clear filename* ttsta* hdrgrid diff_tt
...
for ii = 1:size(pair1,1)
    ...
    %env_trans(ii,:) = env_trans(ii,:)*alpha(ii); % Not necessary
end
