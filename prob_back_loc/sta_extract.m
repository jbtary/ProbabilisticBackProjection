...
%   data: x,y,z data in same order as the complete list of stations
% 

function [sta_name,sta,ind,data,hdr2] = sta_extract(pathandfilename,allsta,grid0,sta_sel)

load(pathandfilename,'hdr','datx','daty','datz')

% Get station info: names and geo locations
if isfield(hdr,'array')
...
for ii = 1:length(sta_name)
    data.x{ii,1} = datx{icomp(ii),1};
    data.y{ii,1} = daty{icomp(ii),1};
    data.z{ii,1} = datz{icomp(ii),1};
...
if ~isempty(sta_sel)
    ...
    for ii = 1:length(sta_name)
        data.x{ii,1} = datasv.x{ia(ii),1};
        data.y{ii,1} = datasv.y{ia(ii),1};
        data.z{ii,1} = datasv.z{ia(ii),1};
