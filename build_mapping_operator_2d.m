% Code to build the mapping operator to locate events: 2D + constant velocity
clear
...
rang = [0 50 0.2;0 50 0.2]; % Search range -- min, max, increments (km)
...
for ii = 1:length(pair1(:,1))
    % Use constant vel
    dist_diff = sqrt( ( sta(pair1(ii,1),1) - sta(pair1(ii,2),1) )^2 + ( sta(pair1(ii,1),2) - sta(pair1(ii,2),2) )^2 );
...
[CC1_ind,alpha] = mapping_var_2d(x_no,y_no,pair1,Npair1,...
    rang(1,1):rang(1,3):rang(1,2),rang(2,1):rang(2,3):rang(2,2),...
    sta,vel,Fs,tlag1);
...
save(name_mapping,'max_samlag','x_no','y_no','CC1_ind','alpha',...
    'rang','pair1','Npair1','sta','sta_name','tlag1','Fs','-v7.3')
